...
2、安装配置：   
2.1、安装,不停next即可；   
2.2、将squid/etc/下的default结尾的文件都去掉default，即变为conf文件   
2.3、根据本机需要配置squid.conf文件（每次修改配置文件后，squid需要重启生效）  
...
（2）确保主机名能够被解析，可以ping主机名进行测试   
    ![image](xftony.github.io/_image/20180426-squid-1.png)

（3）设置配置文件/etc/squid/squid.conf修改如下选项   
    ![image](xftony.github.io/_image/20180426-squid-2.png)

可以找到httpd_port修改代理服务的端口号   
（4）squid -z：初始化如图   
    ![image](xftony.github.io/_image/20180426-squid-3.png)

（5）service squid start：启动服务  
    ![image](xftony.github.io/_image/20180426-squid-4.png)
