...
数据包先经过PREOUTING，由该链确定数据包的走向：  
    ...
    1、目的地址是本地，则发送到INPUT，让INPUT决定是否接收下来送到用户空间，流程为PREROUTIGN--->INPUT;  
    2、若满足PREROUTING的nat表上的转发规则，则发送给FORWARD，然后再经过POSTROUTING发送出去，流程为： PREROUTING--->RORWARD--->POSTROUTING  
